...
총 **11장 (메인 10 + OG 커버 1)**. 아래 정해진 경로에 그대로 떨어뜨리면 페이지에 자동 매칭됩니다. 누락된 슬롯은 `<img onerror>` 폴백으로 자연스럽게 숨겨지므로 일부만 먼저 올라가도 페이지는 정상 동작합니다.

> **변경 이력**: 섹션 브릿지 50장(`sections/{slug}/{traits,signature,power,bridge,slogan}.png`)은 결과 화면에서 제거되어 더 이상 사용하지 않습니다. 디렉터리는 남겨두었지만 비워두셔도 무방합니다.

## 0. 공유 미리보기 — `images/og-cover.png` (1장)

카톡·SMS·트위터·슬랙 등에 페이지 URL이 공유될 때 미리보기 카드로 노출되는 대표 이미지. **카카오톡 SDK 카드와는 별개**(SDK는 결과별 `types/{slug}.png`를 사용).

| 항목 | 사양 |
|---|---|
| 권장 사이즈 | **1200 × 630** (1.91:1) |
| 포맷 | PNG 또는 JPG |
| 용량 목표 | 200~400KB |
| 콘텐츠 | 캠페인 타이틀 "우리 가족은 어떤 가족일까?" + 슬로건 + 입양의날 표시 |
| 안전 영역 | 사방 10% 여백 (썸네일 잘림 방지) |
